<div>
    <div class="mb-8">
        <div class="flex items-center gap-2 mb-2">
            <a href="{{ route('products.index') }}" wire:navigate class="text-sm text-slate-500 hover:text-copper transition-colors">Products</a>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-display font-semibold text-slate-900 tracking-tight">{{ $product->name }}</h1>
                <div class="mt-1 flex items-center gap-3 text-sm text-slate-500">
                    ...
                    @endif
                    <span class="inline-flex items-center rounded-full {{ $product->is_active ? 'bg-teal/10 text-teal-dark' : 'bg-slate-100 text-slate-600' }} px-2.5 py-0.5 text-xs font-medium border {{ $product->is_active ? 'border-teal/20' : 'border-slate-200' }}">
        ...
        <div class="xl:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm">
                <div class="px-6 py-4 border-b border-slate-200">
                    <h2 class="text-base font-display font-semibold text-slate-900">Supplier Pricing</h2>
                            ...
                            <div class="px-6 py-4 hover:bg-slate-50 transition-colors">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-3">
                                        <div class="w-8 h-8 rounded-lg bg-copper/10 flex items-center justify-center">
                                            <x-lucide-building-2 class="w-4 h-4 text-copper" />
                                        </div>
                                        <div>
                                            <div class="flex items-center gap-2">
                                                <span class="text-sm font-medium text-slate-900">{{ $supplier->name }}</span>
                                                @if($supplier->pivot->is_preferred)
                                                    <span class="inline-flex items-center rounded-full bg-teal/10 text-teal-dark border border-teal/20 px-2 py-0.5 text-[10px] font-semibold uppercase">Preferred</span>
                                                @endif
                                            </div>
                                ...
                                @if($supplier->pivot->supplier_product_url)
                                    <a href="{{ $supplier->pivot->supplier_product_url }}" target="_blank" class="text-xs text-copper hover:underline mt-1 ml-11 inline-flex items-center gap-1">
                ...
                @else
                    <div class="p-8 text-center">
                        ...
                        <a href="{{ route('products.edit', $product) }}" wire:navigate class="mt-2 inline-flex items-center gap-1 text-sm font-medium text-copper hover:text-copper">
        ...
        <div class="space-y-6">
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
                ...
                <div class="flex items-center justify-between py-2 border-b border-slate-100">
                    ...
                    <span class="text-sm font-medium {{ $product->stock_qty > 0 ? 'text-teal-dark' : 'text-red-600' }}">{{ $product->stock_qty }}</span>
                </div>
