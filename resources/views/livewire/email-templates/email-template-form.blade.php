<div class="max-w-4xl">
    <div class="mb-8">
        <h1 class="text-2xl font-display font-semibold text-slate-900 tracking-tight">{{ $template ? 'Edit Email Template' : 'Create Email Template' }}</h1>
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-5">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input wire:model="name" id="name" type="text" required class="w-full rounded-lg border-slate-300 text-slate-900 focus:border-copper focus:ring-copper text-sm px-3.5 py-2.5" placeholder="e.g. Quote Sent" />
                    @error('name') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="slug" class="block text-sm font-medium text-slate-700 mb-1.5">Slug <span class="text-red-500">*</span></label>
                    <input wire:model="slug" id="slug" type="text" required class="w-full rounded-lg border-slate-300 text-slate-900 focus:border-copper focus:ring-copper text-sm px-3.5 py-2.5 font-mono" placeholder="quote-sent" />
            ...
            <div>
                ...
                <textarea wire:model="body_text" id="body_text" rows="6" class="w-full rounded-lg border-slate-300 text-slate-900 focus:border-copper focus:ring-copper text-sm px-3.5 py-2.5 font-mono" placeholder="Plain text fallback..."></textarea>
            </div>

            <div>
                <label for="variables_json" class="block text-sm font-medium text-slate-700 mb-1.5">Variables (JSON)</label>
                <textarea wire:model="variables_json" id="variables_json" rows="4" class="w-full rounded-lg border-slate-300 text-slate-900 focus:border-copper focus:ring-copper text-sm px-3.5 py-2.5 font-mono" placeholder='["customer_name", "quote_reference", "grand_total"]'></textarea>
