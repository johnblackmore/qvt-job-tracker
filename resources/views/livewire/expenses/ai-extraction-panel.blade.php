<div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
    ...
    @if(!$isProcessing && !$extraction)
        <div class="border-2 border-dashed border-slate-300 rounded-xl p-8 text-center hover:border-copper/40 transition-colors">
            ...
            <button wire:click="extract" wire:loading.attr="disabled" class="mt-4 inline-flex items-center gap-2 rounded-lg bg-copper px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-copper-dark transition-colors">
                Extract Data
            </button>
        </div>
    @endif
