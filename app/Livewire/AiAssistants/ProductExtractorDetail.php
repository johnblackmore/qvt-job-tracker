...
class ProductExtractorDetail extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        $providerModelStats = AiExtraction::selectRaw('
            ...
            ->map(function ($stat) {
                ...
                if ($config && $config->input_price !== null && $config->output_price !== null) {
                    $inputTokens = AiExtraction::where('provider', $stat->provider)->where('model', $stat->model)->sum('input_tokens');
                    $outputTokens = AiExtraction::where('provider', $stat->provider)->where('model', $stat->model)->sum('output_tokens');
        ...
        return view('livewire.ai-assistants.product-extractor-detail', compact(
            'extractions',
